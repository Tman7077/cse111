...
def make_gui():
     ...
     for i in range(2):
          Grid.rowconfigure(root, i, weight=1, minsize=window_default_h/20)
     for i in range(2, 8):
          Grid.rowconfigure(root, i, weight=2, minsize=window_default_h/10)
     for i in range(4):
          Grid.columnconfigure(root, i, weight=2, minsize=window_default_h/8)
     ...
     input_label.grid(row=1, column=0, columnspan=4, padx=0, pady=2, sticky=N+E+W)

     # input_label is the secondary, smaller number field.
     global past_label
     past_label = Label(root, font=label_font_small, anchor=E, textvariable=past_text,
          bg=dark_colors["bg_dark"], fg=white)
     past_label.grid(row=0, column=1, columnspan=3, sticky=N+E+W)
     ...
     global button_dict
     button_dict = {
          "button_power": ["𝑥ⁿ", lambda: symbol("^"), 2, 0, 'dark_colors["op_dark"]', "KeyPress-asciicircum"],
          "button_clear_entry": ["CE", clr, 2, 1, 'dark_colors["op_dark"]', "Control-KeyPress-BackSpace"],
          "button_clear": ["C", lambda: clr(-1,True), 2, 2, 'dark_colors["op_dark"]', "Control-Shift-KeyPress-BackSpace"],
          "button_backspace": ["⌫", lambda: clr(1), 2, 3, 'dark_colors["op_dark"]', "KeyPress-BackSpace"],
          
          "button_inverse": ["⅟𝑥", inverse, 3, 0, 'dark_colors["op_dark"]', None],
          "button_sq": ["𝑥²", lambda: sq(), 3, 1, 'dark_colors["op_dark"]', None],
          "button_sqrt": ["√", lambda: sq(rt), 3, 2, 'dark_colors["op_dark"]', None],
          "button_divide": ["÷", lambda: symbol("÷"), 3, 3, 'dark_colors["op_dark"]', ["KeyPress-slash", "KeyPress-KP_Divide"]],
          
          "button_7": ["7", lambda: button_num(7), 4, 0, 'dark_colors["num_dark"]', ["KeyPress-7", "KeyPress-KP_7"]],
          "button_8": ["8", lambda: button_num(8), 4, 1, 'dark_colors["num_dark"]', ["KeyPress-8", "KeyPress-KP_8"]],
          "button_9": ["9", lambda: button_num(9), 4, 2, 'dark_colors["num_dark"]', ["KeyPress-9", "KeyPress-KP_9"]],
          "button_multiply": ["×", lambda: symbol("×"), 4, 3, 'dark_colors["op_dark"]', ["KeyPress-x", "KeyPress-asterisk", "KeyPress-KP_Multiply"]],
          
          "button_4": ["4", lambda: button_num(4), 5, 0, 'dark_colors["num_dark"]', ["KeyPress-4", "KeyPress-KP_4"]],
          "button_5": ["5", lambda: button_num(5), 5, 1, 'dark_colors["num_dark"]', ["KeyPress-5", "KeyPress-KP_5"]],
          "button_6": ["6", lambda: button_num(6), 5, 2, 'dark_colors["num_dark"]', ["KeyPress-6", "KeyPress-KP_6"]],
          "button_subtract": ["-", lambda: symbol("-"), 5, 3, 'dark_colors["op_dark"]', ["minus", "KP_Subtract"]],
          
          "button_1": ["1", lambda: button_num(1), 6, 0, 'dark_colors["num_dark"]', ["KeyPress-1", "KeyPress-KP_1"]],
          "button_2": ["2", lambda: button_num(2), 6, 1, 'dark_colors["num_dark"]', ["KeyPress-2", "KeyPress-KP_2"]],
          "button_3": ["3", lambda: button_num(3), 6, 2, 'dark_colors["num_dark"]', ["KeyPress-3", "KeyPress-KP_3"]],
          "button_add": ["+", lambda: symbol("+"), 6, 3, 'dark_colors["op_dark"]', ["KeyPress-plus", "KeyPress-KP_Add"]],
          
          "button_negate": ["±", negate, 7, 0, 'dark_colors["op_dark"]', None],
          "button_0": ["0", lambda: button_num(0), 7, 1, 'dark_colors["num_dark"]', ["KeyPress-0", "KeyPress-KP_0"]],
          "button_decimal": [".", lambda: button_num("."), 7, 2, 'dark_colors["op_dark"]', ["KeyPress-period", "KeyPress-KP_Decimal"]],
          "button_equals": ["=", equals, 7, 3, 'dark_colors["equals_dark"]', ["KeyPress-equal", "KeyPress-Return", "KeyPress-KP_Enter", "KeyPress-KP_Equal"]]
     }
     ...
     for button in button_dict.keys():
          ...
          globals()[button] = Button(root, font=button_font, cursor="hand2", bg=bcolorbg,
          ...
          this line grids it.'''
          eval(button).grid(row=brow, column=bcol, sticky="nesw", padx=1, pady=1)
...
def equals():
     if not making_gui:
          ...
          if answer > 10 ** 12 or answer < -(10 ** 16):
               current_text.set(float(answer))
          elif answer < 10 ** -4 and answer > -(10 ** -4):
               current_text.set(float(answer))
          else:
               ...
               # Otherwise, round the answer to 10 decimals
                    ...
                    current_text.set(round(answer, 10))
...
# Handle reading of scientific notation
# Handle complex numbers?
