...
def make_gui():
     ...
     # Define some colors
     global black, white, dark_colors, light_colors, color_palette
     black = "#000000"
     white = "#FFFFFF"
     dark_colors = {
          "bg_dark": "#21201e",
     ...
     root.config(bg=dark_colors["bg_dark"])
     ...
     mode_button = Button(root, font=button_font, textvariable=mode_text, width=3,
          bg=dark_colors["bg_dark"], fg=white, command=change_color_palette)
...
def change_color_palette(r=root):
     if not making_gui:
          global color_palette
          for button in button_dict.keys():
               if color_palette == "dark_colors":
                    bcolorbg = eval(button_dict[button][4].replace("dark", "light"))
                    bcolorfg = black
                    if bcolorbg == light_colors["equals_light"]:
                         bcolorfg = white
               elif color_palette == "light_colors":
                    bcolorbg = eval(button_dict[button][4].replace("light", "dark"))
                    bcolorfg = white
                    if bcolorbg == dark_colors["equals_dark"]:
                         bcolorfg = black
               eval(button).config(bg=bcolorbg, fg=bcolorfg)
          widgets = [mode_button, past_label, input_label]
          frames = [b_frame, il_frame]
          if color_palette == "dark_colors":
               bgl = light_colors["bg_light"]
               r.config(bg=bgl)
               for widget in widgets:
                    widget.config(bg=bgl, fg=black)
               for frame in frames:
                    frame.config(bg=bgl)
               mode_text.set("☾")
               color_palette = "light_colors"
          elif color_palette == "light_colors":
               bgd = dark_colors["bg_dark"]
               r.config(bg=bgd)
               for widget in widgets:
                    widget.config(bg=bgd, fg=white)
               for frame in frames:
                    frame.config(bg=bgd)
               mode_text.set("☼")
               color_palette = "dark_colors"
