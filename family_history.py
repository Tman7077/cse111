...
def main():
    ...
    # Call the print_num_marriages function to print
    # human readable data about the number of marriages.
    print_num_marriages(marriages_dict, people_dict)
...
def print_death_age(people_dict):
    ...
    for person in people_dict:
        age = people_dict[person][3] - people_dict[person][2]
        print(f'{people_dict[person][0]}: {age} ({people_dict[person][2]} - {people_dict[person][3]})')
...
def print_marriages(marriages_dict, people_dict):
    ...
    for marriage in marriages_dict:
        wed_yr = marriages_dict[marriage][2]
        hub = marriages_dict[marriage][0]
        wife = marriages_dict[marriage][1]
        print(f'{people_dict[hub][0]} ({wed_yr - people_dict[hub][2]}), was married to',
            f'{people_dict[wife][0]} ({wed_yr - people_dict[wife][2]}), in {wed_yr}.')

def print_num_marriages(marriages_dict, people_dict):
    """Determine and print the number of marriages 
    each person in people_dict has.
    ...
    Return: nothing
    """
    pdict_copy = people_dict.copy()
    for person in pdict_copy:
        pdict_copy[person].append(0)

    for marriage in marriages_dict:
        hub = marriages_dict[marriage][0]
        wife = marriages_dict[marriage][1]
        hub_m = pdict_copy[hub][4]
        wife_m = pdict_copy[wife][4]
        hub_m += 1
        wife_m += 1
        pdict_copy[hub][4] = hub_m
        pdict_copy[wife][4] = wife_m
        
    for person in pdict_copy:
        if pdict_copy[person][4] != 1:
            time_s = "times"
        else:
            time_s = "time"
        print(f'{pdict_copy[person][0]} was married',
        f'{pdict_copy[person][4]} {time_s}.')
